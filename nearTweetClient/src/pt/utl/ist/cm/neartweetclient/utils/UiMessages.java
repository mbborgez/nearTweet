package pt.utl.ist.cm.neartweetclient.utils;

/**
 * 
 * Ideally it should read from values according with the language selected
 * 
 * see more on: http://developer.android.com/guide/topics/resources/localization.html
 *
 */
public class UiMessages {
	
	public static final String ENTER_LOGIN = "You should enter a valid name to proceed";
	public static final String INVALID_LOGIN = "The current name: %s is not accessible!";
	public static final String WELCOME_MESSAGE = "Welcome to NearTweet %s!!";
	public static final String ERROR_MESSAGE = "Cannot establish connection to the server!";
	public static final String ERROR_CONNECTING_MESSAGE = "Error connecting";
	public static final String NEARTWEET_TAG = "NearTweet";

}
